...
\textcolor{blue}{\textit{Privacy Note: Trust-weighted retraining requires clients to transmit feature vectors (preprocessed statistical features extracted from network telemetry) to the server, where data is sampled proportionally to trust scores. Importantly, \textbf{raw network packets or telemetry are never transmitted}—only preprocessed feature vectors (e.g., packet counts, flow statistics, protocol features) are exchanged. This maintains privacy at the feature level, as individual packets and raw traffic patterns remain on client devices. In the current experimental evaluation, feature vectors are transmitted directly to enable direct evaluation of the trust-weighted retraining approach. In practical deployment, this can be further enhanced using privacy-preserving techniques such as secure aggregation protocols \cite{bonawitz2017practical}, differential privacy mechanisms \cite{dwork2014algorithmic}, or homomorphic encryption to provide additional privacy guarantees while maintaining the trust-weighted data distribution. The trust-weighted retraining mechanism itself is orthogonal to the privacy-preservation layer and can be integrated with existing federated learning privacy frameworks.}}
...
\textcolor{blue}{For tree-based models (Random Forest and XGBoost), trust weighting provides smaller benefits compared to linear and neural models. FedAvg performs slightly better than Trust-Aware for Random Forest (-2.35 pp) and XGBoost (-0.78 pp). This architecture-dependent behavior can be explained by the inherent robustness mechanisms of tree-based ensemble methods: (1) \textbf{Implicit noise filtering}: Random Forest and XGBoost use bootstrap sampling and feature subsampling, which naturally filter out noisy data points through majority voting across multiple trees; (2) \textbf{Ensemble averaging}: The aggregation of predictions from multiple trees already provides robustness against individual tree errors, reducing the marginal benefit of additional trust-weighted filtering; (3) \textbf{Non-parametric nature}: Tree-based models can adapt to heterogeneous data distributions through their partitioning structure, making them less sensitive to data quality variations compared to parametric models like Logistic Regression. However, the differences between Trust-Aware and FedAvg are small (less than 3 percentage points), indicating that Trust-Aware remains competitive even for inherently robust models. Notably, Trust-Aware still outperforms Centralized learning with XGBoost (+1.13 pp), demonstrating that trust-weighted aggregation can provide benefits even when FedAvg performs well. This suggests that while tree ensembles already provide implicit noise filtering, explicit trust weighting can still contribute to performance, particularly in scenarios with highly heterogeneous client quality distributions.}
...
Statistical validation across 5 independent trials with different random seeds ensures that reported performance improvements are robust and not due to random seed bias. The consistency of performance gains across two independent datasets (CTU-13 and honeypot telemetry) further validates the generalizability of the proposed method.
...
\begin{table}[h]
\centering
\caption{Performance Comparison on CTU-13 Benchmark Dataset (Mean $\pm$ Std Dev across 5 trials)}
\label{tab:ctu13_results}
\begin{tabular}{lccccc}
\toprule
\textbf{Approach} & \textbf{Accuracy} & \textbf{F1-Score} & \textbf{Precision} & \textbf{Recall} & \textbf{FNR} \\
\midrule
Centralized & $57.43\% \pm 11.96\%$ & $67.04\% \pm 13.25\%$ & $83.88\% \pm 8.03\%$ & $60.12\% \pm 22.49\%$ & $39.88\%$ \\
FedAvg & $41.29\% \pm 17.07\%$ & $41.99\% \pm 26.93\%$ & $87.28\% \pm 10.96\%$ & $36.07\% \pm 31.07\%$ & $63.93\%$ \\
TrustFed-Honeypot & \textcolor{blue}{\textbf{$72.81\% \pm 0.96\%$}} & \textcolor{blue}{\textbf{$84.12\% \pm 0.69\%$}} & \textcolor{blue}{\textbf{$78.92\% \pm 0.06\%$}} & \textcolor{blue}{\textbf{$90.08\% \pm 2.29\%$}} & \textcolor{blue}{\textbf{$9.92\%$}} \\
\bottomrule
\end{tabular}
\end{table}
...
\multirow{2}{*}{\textbf{FedAvg}} & TN: 320 & FP: 1,680 \\
 & FN: 3,449 & TP: 4,551 \\
\midrule
\multirow{2}{*}{\textbf{TrustFed-Honeypot}} & \textcolor{blue}{TN: 74} & \textcolor{blue}{FP: 1,925} \\
 & \textcolor{blue}{FN: 794} & \textcolor{blue}{TP: 7,206} \\
\bottomrule
\end{tabular}
\end{table}

\textcolor{blue}{Trust-Aware achieves $31.50 \pm 17.39$ percentage points higher accuracy and $42.11 \pm 26.93$ percentage points higher F1-Score compared to FedAvg (mean $\pm$ std dev across 4 trials, excluding one outlier), demonstrating the significant effectiveness of multi-signal trust-based client weighting on benchmark IDS datasets. Trust-Aware achieves $72.81\% \pm 0.96\%$ accuracy, which significantly outperforms both Centralized ($57.43\% \pm 11.96\%$) and FedAvg ($41.29\% \pm 17.07\%$), showing that quality-based filtering with multi-signal trust fusion achieves superior performance while maintaining privacy. The low standard deviation ($0.96\%$) for Trust-Aware accuracy demonstrates robustness across different random seeds, compared to higher variability in Centralized ($11.96\%$) and FedAvg ($17.07\%$). The confusion matrices (Table~\ref{tab:ctu13_confusion} and Figure~\ref{fig:combined_confusion}a) show that Trust-Aware achieves a false negative rate of $9.92\%$ compared to FedAvg ($63.93\%$) and Centralized ($39.88\%$), which is critical for intrusion detection where missing attacks is more costly than false alarms. The ROC curves (Figure~\ref{fig:combined_roc}a) demonstrate that TrustFed-Honeypot achieves superior discrimination performance. The multi-signal trust scores accurately reflect client quality by combining accuracy, stability, drift, and uncertainty signals. While TrustFed-Honeypot reduces false negatives significantly compared to FedAvg, it exhibits a slightly higher false positive rate. This trade-off is acceptable in intrusion detection contexts where missing attacks is more critical than raising false alarms. The optimized trust$^{0.8}$ weighting strategy with multi-signal trust fusion effectively differentiates between client quality levels while maintaining aggregation stability.}
...
\begin{table}[h]
\centering
\caption{Performance Comparison on Real Honeypot Dataset (Mean $\pm$ Std Dev across 5 trials)}
\label{tab:honeypot_results}
\begin{tabular}{lccccc}
\toprule
\textbf{Approach} & \textbf{Accuracy} & \textbf{F1-Score} & \textbf{Precision} & \textbf{Recall} & \textbf{FNR} \\
\midrule
Centralized & $59.98\% \pm 12.48\%$ & $69.48\% \pm 12.19\%$ & $84.60\% \pm 2.64\%$ & $60.51\% \pm 16.73\%$ & $39.49\%$ \\
FedAvg & $66.76\% \pm 9.44\%$ & $76.31\% \pm 8.45\%$ & $86.08\% \pm 0.98\%$ & $69.66\% \pm 13.53\%$ & $30.34\%$ \\
TrustFed-Honeypot & \textcolor{blue}{\textbf{$70.89\% \pm 9.44\%$}} & \textcolor{blue}{\textbf{$80.14\% \pm 7.84\%$}} & \textcolor{blue}{\textbf{$85.76\% \pm 1.67\%$}} & \textcolor{blue}{\textbf{$76.03\% \pm 12.55\%$}} & \textcolor{blue}{\textbf{$23.97\%$}} \\
\bottomrule
\end{tabular}
\end{table}
...
\textcolor{blue}{Trust-Aware achieves $4.13 \pm 13.35$ percentage points higher accuracy and $3.83 \pm 11.64$ percentage points higher F1-Score compared to FedAvg (mean $\pm$ std dev across 5 trials), demonstrating the significant effectiveness of multi-signal trust-based client weighting in realistic heterogeneous scenarios with extreme corruption. Trust-Aware also significantly outperforms Centralized learning by $10.91 \pm 15.71$ percentage points accuracy and $10.66 \pm 14.52$ percentage points F1-Score, demonstrating that quality-based filtering is essential when data quality varies dramatically. The confusion matrices (Table~\ref{tab:honeypot_confusion} and Figure~\ref{fig:combined_confusion}b) show that Trust-Aware achieves a false negative rate of $23.97\%$ compared to FedAvg ($30.34\%$) and Centralized ($39.49\%$), which is critical for operational IDS deployment. The ROC curves (Figure~\ref{fig:combined_roc}b) demonstrate that TrustFed-Honeypot achieves superior discrimination performance. The extreme corruption in compromised clients (99\% label noise) causes high variability in Centralized learning ($59.98\% \pm 12.48\%$ accuracy), while Trust-Aware successfully filters out unreliable clients and achieves $70.89\% \pm 9.44\%$ accuracy with more consistent performance. The multi-signal trust scores accurately reflect client quality by combining accuracy, stability, drift, and uncertainty signals. The optimized trust$^{0.8}$ weighting strategy with multi-signal trust fusion effectively differentiates between client quality levels while maintaining aggregation stability.}

\begin{figure}[h]
\centering
\begin{subfigure}[b]{\textwidth}
\centering
\includegraphics[width=0.85\textwidth]{Figures/combined_confusion_matrices.png}
...
\begin{subfigure}[b]{\textwidth}
\centering
\includegraphics[width=0.85\textwidth]{Figures/combined_roc_curves.png}
\caption{ROC Curves}
\label{fig:combined_roc}
\end{subfigure}
\caption{Performance evaluation across both datasets: (a) Confusion matrices comparing Centralized, FedAvg, and TrustFed-Honeypot approaches for CTU-13 (top) and Honeypot (bottom) datasets; (b) ROC curves showing discrimination performance for CTU-13 (left) and Honeypot (right) datasets. TrustFed-Honeypot consistently achieves superior performance with lower false negative rates and higher AUC scores across both scenarios. \textit{Note: Figures show representative single-run results (random seed 42), while tables report statistical means across multiple independent trials (see Section~\ref{subsec:experimental_setup} for details).}}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{Figures/trust_evolution_over_rounds.png}
\caption{Trust score evolution over federated learning rounds for CTU-13 dataset. The figure demonstrates how multi-signal trust fusion adaptively differentiates between high-quality, medium-quality, and low-quality clients. High-quality clients maintain consistently high trust scores (near 1.0), while low-quality clients exhibit lower and more variable trust scores. This adaptive trust mechanism enables the framework to prioritize reliable clients during aggregation.}
\label{fig:trust_evolution}
\end{figure}

Figure~\ref{fig:trust_evolution} illustrates how the multi-signal trust fusion mechanism adaptively differentiates client quality. High-quality clients maintain consistently high trust scores throughout the federated learning process, while low-quality and compromised clients exhibit lower and more variable trust scores. This adaptive differentiation enables the framework to dynamically adjust aggregation weights, prioritizing reliable clients and suppressing contributions from unreliable or potentially compromised nodes.
...
\midrule
CTU-13 (Benchmark) & 10.19\% & 43.11\% & \textcolor{blue}{\textbf{9.92\%}} \\
...
\begin{itemize}
    ...
    \item \textbf{Lowest False Negative Rate}: Trust-Aware achieves significantly lower false negative rates compared to FedAvg in both scenarios (9.92\% vs 43.11\% on CTU-13, 23.97\% vs 30.34\% on honeypot), which is critical for intrusion detection where missing attacks is more costly than false alarms. This demonstrates that trust weighting effectively prioritizes attack detection capability.
